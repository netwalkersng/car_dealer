...
if ( ! function_exists('show_price'))
{
	function show_price ($price,$contact_for_price='')
	{
		...
		$currency_placing = get_settings('content_settings','currency_placing','before_with_no_gap');
...
if ( ! function_exists('show_package_price'))
{
	function show_package_price ($price)
	{
		...
		$currency_placing = get_settings('content_settings','currency_placing','before_with_no_gap');
		$currency_type = get_settings('content_settings','system_currency_type','1');
		...
		{
			return $currency.''.number_format($price,2,$decimal_point, $thousand_separator);
		}
		else if($currency_placing=='before_with_gap')
		{
			return $currency.' '.number_format($price,2,$decimal_point, $thousand_separator);
		...
		{
			return number_format($price,2,$decimal_point, $thousand_separator).''.$currency;
		}
		else
		{
			return number_format($price,2,$decimal_point, $thousand_separator).' '.$currency;
...
if ( ! function_exists('get_all_currencies'))
{
	function get_all_currencies($key=0)
	{
		$currencies= array(
			...
			"ZWD"=> array("carbizbabwe, carbizbabwe Dollars", "5a, 24"));
...
if ( ! function_exists('get_status_title_by_value'))
{
	...
	{
		$types = array("STATUS_DELETED","STATUS_ACTIVE","STATUS_PENDING","STATUS_PAYMENT_PENDING","STATUS_EXPIRED");
		
		$front = '';
		$back  = '</div>';
		if($types[$key]=='STATUS_DELETED')
		{
			$front = '<div class="label-status label label-danger">';
		}
		else if($types[$key]=='STATUS_ACTIVE')
		{
			$front = '<div class="label-status label label-success">';
		}
		else if($types[$key]=='STATUS_PENDING')
		{
			$front = '<div class="label-status label label-primary">';
		}
		else if($types[$key]=='STATUS_PAYMENT_PENDING')
		{
			$front = '<div class="label-status label label-primary">';
		}
		else if($types[$key]=='STATUS_EXPIRED')
		{
			$front = '<div class="label-status label label-error">';
		}
		return (isset($types[$key]))?$front.$types[$key].$back:'N/A';
	}
}
...
if ( ! function_exists('social_sharing_meta_tags_for_post'))
{
	...
	function social_sharing_meta_tags_for_post($post='')
	{
		if($post!='' && $post->num_rows()>0)
		{
			...
			$site_title = get_settings('site_settings','site_title','carbiz');
			...
			if($fb_app_id!='' && $fb_app_id!='none')
				$meta .= '<meta property="fb:app_id" content="'.get_settings('content_settings','fb_app_id','none').'" />';
    ...
    function social_sharing_meta_tags_for_blog($blog_meta='')
    {
        if($blog_meta!='')
        {
            ...
            $meta = '<meta name="twitter:card" content="photo" />'."\n".
                ...
                '<link rel="canonical" href="'.$detail_link.'"/>'."\n".
                '<meta name="description" content="'.$description.'">'."\n".
			    '<meta name="keywords" content="'.get_settings('site_settings', 'key_words', 'Search,list,get,car,local,area').'"/>';
...
if ( ! function_exists('get_category_title_icon_by_id'))
{
	function get_category_title_icon_by_id($id='')
	{
		if($id==0)
			return array('title'=>'none','icon'=>'default.png');
		...
		{
			$row = $query->row();
			return array('title'=>lang_key($row->title),'icon'=>$row->featured_img);
		}
		else
			return array('title'=>lang_key('none'),'icon'=>'default.png');
	}
}
...
if ( ! function_exists('get_all_car_map_data'))
{
    function get_all_car_map_data()
    {
        $CI = get_instance();
        $CI->load->database();
        $query 	= $CI->db->get_where('posts',array('status'=> 1),100,0);
        $data 	= array();
        $posts 	= array();
        ...
        foreach ($query->result() as $row)
        {
			...
			$post['price'] 				= ($row->ask_for_price==1)?lang_key('ask_for_price'):$row->price;
			$post['post_short_address'] = str_replace("'",'',lang_key(get_location_name_by_id($row->city)));
			$post['detail_link'] 		= post_detail_url($row);

			$cat_info = get_category_title_icon_by_id($row->category);
			$post['parent_category'] 	= $cat_info['title'];
			$post['fa_icon'] 			= get_car_type_icon("link",$cat_info['icon']);
...
if ( ! function_exists('get_car_map_data_single'))
{
	function get_car_map_data_single($row)
	{
		$data 	= array();
		$posts 	= array();
...
if ( ! function_exists('expiration_cron'))
{
	function expiration_cron ()
	{
		...
		if((is_array($option) && isset($option['error'])) || strtotime($option->values)<strtotime($today))
		{
			...
			if(get_settings('content_settings','hide_posts_if_expired','Yes')=='Yes')
...
if ( ! function_exists('send_expiration_notification'))
{
	function send_expiration_notification($expirtion_date='')
	{
		...
		if($CI->config->item('send_notification_before_post_expiration')=='yes')
		{
			...
			$link = site_url('admin/content/allposts');
...
if ( ! function_exists('get_brand_model_name_by_id'))
{
    function get_brand_model_name_by_id ($id)
    {
        ...
        $CI = get_instance();
        $query = $CI->db->get_where('carbiz_brandmodels',array('id'=>$id));
        if($query->num_rows()>0)
        {
            $row = $query->row();
            return $row->name;
...
if ( ! function_exists('get_all_brands'))
{
	function get_all_brands ()
	{
		...
		$query = $CI->db->get_where('carbiz_brandmodels',array('type'=>'brand','status'=>1));
		return $query;
	}
}
...
if ( ! function_exists('get_all_models'))
{
	function get_all_models ($brand='')
	{
		...
		$query = $CI->db->get_where('carbiz_brandmodels',array('type'=>'model','status'=>1));
		return $query;
	}
}
...
if ( ! function_exists('get_payment_status_title_by_value'))
{
	...
	{
		$types = array("STATUS_DELETED","STATUS_ACTIVE","STATUS_PENDING");
		if($label=='labelled') {
			if($key==0) {
				return '<span class="label label-danger">'.$types[$key].'</span>';
			}
			else if($key==1) {
				return '<span class="label label-success">'.$types[$key].'</span>';
			}
			return (isset($types[$key]))?'<span class="label label-warning">'.$types[$key].'</span>':'N/A';
		}
		return (isset($types[$key]))?$types[$key]:'N/A';
	}
}
